...
def closer_city(lat, lon, city_a, city_b):
    ...
    closer_city = min([city_a,city_b],key = lambda city: distance(city,target_city))
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        ...
        if sum([berry_finder(b) for b in branches(t)]) > 0: 
            return True
        else:
            return False
# sum[True,True] == 2
...
def construct_sent(word, table):
    ...
    while word not in ['.', '!', '?']:
        "*** YOUR CODE HERE ***"
        result = result + ' ' + word
        word = random.choice(table[word]) # random.choice(item) 随机选择一个item
    return result.strip() + word # strip() 过滤首尾空格
