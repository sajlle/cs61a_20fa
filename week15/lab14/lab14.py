...
def prune_min(t):
    ...
    if t.is_leaf():
        ...
        label_l, label_r = t.branches[0].label,t.branches[1].label 
        if label_l <= label_r:
            del t.branches[1]
        else:
            del t.branches[0]
    ...
    if lst == Link.empty:
        ...
        def func(lst1, num):
            if lst1 == Link.empty:
                ...
                if pred(num):
                    temp_node, temp_lst = Link(num), lst1
                    while temp_lst.rest != Link.empty:
                        temp_lst = temp_lst.rest
                    temp_lst.rest = temp_node
                return lst1
        return foldl(lst, func, Link.empty)
