...
def balanced(m):
    ...
    if is_planet(m):
        ...
        l_t,r_t = length(left(m))*total_weight(end(left(m))), length(right(m))*total_weight(end(right(m)))
        return l_t == r_t and balanced(end(left(m))) and balanced(end(right(m)))
...
def replace_leaf(t, find_value, replace_value):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        val = replace_value if label(t) == find_value else label(t)
        return tree(val)
    else:
...
def has_path(t, word):
    ...
    if len(word) == 1:
        ...
        res = [has_path(b,word[1:]) for b in branches(t) if word[0] == label(t)]
        return True in res
