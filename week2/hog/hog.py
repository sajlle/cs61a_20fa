...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    # assert opponent_score >= 0, 'Must input a nonnegative integer'
    ones = opponent_score % 10
    tens = opponent_score % 100 // 10
    pig_tail = abs(tens - ones) * 2 + 1
    return pig_tail
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score1 = update(num_rolls, score1, score0, dice) # 错在此处，原来写成了score1 = update(num_rolls, score1, score0, dice=six_sided)所以才报错
        who = 1 - who
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    # always run 5 returns a number 5
    return lambda x,y: n
    # END PROBLEM 6
...
def make_averaged(original_function, total_samples=1000):
    ...
    def total_averaged(*arg):
        ...
        for i in range(total_samples):
            curr_value = original_function(*arg)
            sum += curr_value
        average = sum / total_samples
        return average
    return total_averaged
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for i in range(1,11):
        curr_average = averaged_dice(i,dice)
        # print(curr_average)
        list.append(curr_average) 
    max_average = max(list)
    for j in range(10):
        if list[j] == max_average:
            return j+1  
    # END PROBLEM 9
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    pig_square_score=square_update(0, score, opponent_score, dice=six_sided)
    earned_score = pig_square_score - score
    if earned_score >= threshold:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
